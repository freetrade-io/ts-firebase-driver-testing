...
describe("In-process Firestore offset queries on a collectionGroup", () => {
    ...
    test("offset less than total size", async () => {
        // Given some data in a collectionGroup;
        await db.collection("zooA/animals").add({ name: "cat" })
        await db.collection("zooA/animals").add({ name: "ant" })
        await db.collection("zooB/animals").add({ name: "bee" })
    ...
    test("offset composes with limit for pagination", async () => {
        // Given some data in a collectionGroup;
        await db.collection("zooA/animals").add({ name: "dog" })
        await db.collection("zooA/animals").add({ name: "cat" })
        await db.collection("zooB/animals").add({ name: "ant" })
        await db.collection("zooB/animals").add({ name: "bee" })
